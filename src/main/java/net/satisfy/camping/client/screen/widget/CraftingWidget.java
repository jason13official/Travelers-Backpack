...
import net.satisfy.camping.compat.craftingtweaks.ICraftingTweaks;
import net.satisfy.camping.compat.craftingtweaks.TravelersBackpackCraftingGridAddition;
...
public class CraftingWidget extends WidgetBase
{
    ...
    public void drawMouseoverTooltip(DrawContext drawContext, int mouseX, int mouseY)
    {
        ...
        if(isMouseOver(mouseX, mouseY) && showTooltip && isVisible)
        {
            if(!isWidgetActive())
            {
                drawContext.drawTooltip(screen.getTextRenderer(), Text.translatable("screen.camping.crafting"), mouseX, mouseY);
            }
            else
            {
                ...
                if(mouseX >= x + 6 && mouseY >= y + 87 && mouseX < x + 16 && mouseY < y + 97)
                {
                    if(screen.inventory.getSettingsManager().shiftClickToBackpack())
                    {
                        drawContext.drawTooltip(screen.getTextRenderer(), Text.translatable("screen.camping.crafting_to_backpack"), mouseX, mouseY);
                    }
                    else
                    {
                        drawContext.drawTooltip(screen.getTextRenderer(), Text.translatable("screen.camping.crafting_to_player"), mouseX, mouseY);
                    }
                }
            }
        }
