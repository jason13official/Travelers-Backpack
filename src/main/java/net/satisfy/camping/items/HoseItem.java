...
public class HoseItem extends Item
{
                ...
                if(world.canPlayerModifyAt(player, blockpos) && player.canPlaceOn(blockpos1, direction1, stack))
                {
                    ...
                    if(blockstate1.getBlock() instanceof FluidDrainable drainable)
                    {
                        ...
                        if(fluid != Fluids.EMPTY && blockstate1.getFluidState().isStill())
                        {
                            ...
                            boolean canFill = tank.isResourceBlank() || tank.getResource().getFluid().matchesType(fluidVariant.getFluid());
                    ...
                    try(Transaction transaction = Transaction.openOuter())
                    {
                        ...
                        if(fluidVariant != null && !fluidVariant.isBlank())
                        {
                            ...
                            if(amountInserted == FluidConstants.BUCKET && amountExtracted == FluidConstants.BUCKET)
                            {
                                ...
                                world.playSound(player, pos, FluidVariantAttributes.getFillSound(fluidVariant), SoundCategory.BLOCKS, 1.0F, 1.0F);
                                inv.markDataDirty(ITravelersBackpackInventory.TANKS_DATA);
                                transaction.commit();
                ...
                if(!tank.isResourceBlank())
                {
                    ...
                    if(world.getBlockState(newPos).canBucketPlace(fluid))
                    {
                        ...
                        if(tank.getAmount() >= FluidConstants.BUCKET)
                        {
                            ...
                            if(world.setBlockState(newPos, fluidVariant.getFluid().getDefaultState().getBlockState()))
                            {
                                try (Transaction transaction = Transaction.openOuter()) {
                                    long amountExtracted = tank.extract(tank.getResource(), FluidConstants.BUCKET, transaction);
                                    if (amountExtracted == FluidConstants.BUCKET) {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context)
    {
        ...
        {
            tooltip.add(Text.translatable("hose.camping.not_assigned").formatted(Formatting.BLUE));
        }
        else
        {
            if(stack.getNbt() != null)
            {
                ...
                {
                    tooltip.add(Text.translatable("hose.camping.current_mode_suck").formatted(Formatting.BLUE));
                }

                if(compound.getInt("Mode") == SPILL_MODE)
                {
                    tooltip.add(Text.translatable("hose.camping.current_mode_spill").formatted(Formatting.BLUE));
                }

                if(compound.getInt("Mode") == DRINK_MODE)
                {
                    tooltip.add(Text.translatable("hose.camping.current_mode_drink").formatted(Formatting.BLUE));
                }

                if(compound.getInt("Tank") == 1)
                {
                    tooltip.add(Text.translatable("hose.camping.current_tank_left").formatted(Formatting.BLUE));
                }

                if(compound.getInt("Tank") == 2)
                {
                    tooltip.add(Text.translatable("hose.camping.current_tank_right").formatted(Formatting.BLUE));
                }
            }
        }
    }
    ...
    public Text getName(ItemStack stack)
    {
        ...
        String mode = "";
        String localizedName = Text.translatable("item.camping.hose").getString();
        String suckMode = Text.translatable("item.camping.hose.suck").getString();
        String spillMode = Text.translatable("item.camping.hose.spill").getString();
        String drinkMode = Text.translatable("item.camping.hose.drink").getString();
