...
public class UpgradeItem extends Item
{
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context)
    {
        if(TravelersBackpackConfig.getConfig().backpackSettings.enableTierUpgrades)
        {
            ...
            {
                case BLANK_UPGRADE:
                    tooltip.add(Text.translatable("item.camping.blank_upgrade_tooltip").formatted(Formatting.BLUE));
                    break;
                case IRON_TIER_UPGRADE:
                    tooltip.add(Text.translatable("item.camping.tier_upgrade_tooltip", Tiers.LEATHER.getName()).formatted(Formatting.BLUE));
                    break;
                case GOLD_TIER_UPGRADE:
                    tooltip.add(Text.translatable("item.camping.tier_upgrade_tooltip", Tiers.IRON.getName()).formatted(Formatting.BLUE));
                    break;
                case DIAMOND_TIER_UPGRADE:
                    tooltip.add(Text.translatable("item.camping.tier_upgrade_tooltip", Tiers.GOLD.getName()).formatted(Formatting.BLUE));
                    break;
                case NETHERITE_TIER_UPGRADE:
                    tooltip.add(Text.translatable("item.camping.tier_upgrade_tooltip", Tiers.DIAMOND.getName()).formatted(Formatting.BLUE));
                    break;
            }
        }
        else
        {
            ...
            {
                tooltip.add(Text.translatable("item.camping.upgrade_disabled"));
            }
        }
        ...
        if(type == Upgrade.CRAFTING_UPGRADE)
        {
            ...
            {
                tooltip.add(Text.translatable("item.camping.crafting_upgrade_tooltip").formatted(Formatting.BLUE));
            }
            else
            {
                tooltip.add(Text.translatable("item.camping.upgrade_disabled").formatted(Formatting.RED));
            }
        }
    }
