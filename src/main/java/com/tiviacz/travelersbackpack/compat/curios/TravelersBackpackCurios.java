...
public class TravelersBackpackCurios
{
    ...
    public static boolean renderCurioLayer(AbstractClientPlayer clientPlayer)
    {
        ...
        {
            return CuriosApi.getCuriosHelper().findFirstCurio(clientPlayer, stack -> stack.getItem() instanceof TravelersBackpackItem)
                    .map(result -> result.slotContext().visible()).orElse(false);
        }
        return false;
    }
    ...
    public static boolean rightClickEquip(Player player, ItemStack stack, boolean simulate)
    {
        ...
        {
            ICurio curio = CuriosApi.getCuriosHelper().getCurio(stack).resolve().get();
