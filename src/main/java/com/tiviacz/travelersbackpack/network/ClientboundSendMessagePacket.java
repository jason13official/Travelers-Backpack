    ...
    public static void handle(final ClientboundSendMessagePacket message, Supplier<NetworkEvent.Context> ctx)
    {
        ctx.get().enqueueWork(() ->
        {
            if(TravelersBackpackConfig.CLIENT.sendBackpackCoordinatesMessage.get())
            {
                ...
                {
                    Minecraft.getInstance().player.sendMessage(new TranslatableComponent(message.drop ? "information.travelersbackpack.backpack_drop" : "information.travelersbackpack.backpack_coords", message.pos.getX(), message.pos.getY(), message.pos.getZ()), Minecraft.getInstance().player.getUUID());
                }
            }
        });
